<?php
	...
	if(empty($_POST) ===false){
		...
		$id = $_POST['ID_COURS'];
		$type_cours = get_value($conn, 'Cours_Profs', $id, 'TYPE_COURS');

		if($type_cours == 'Groupe'){

			$cout= get_prices($conn, 'Tarification');
			foreach ($cout as $ligne) {
				if($ligne['CATEGORIE'] == 'PROF_PRIX_GROUPE'){
			...
			$cout= get_prices($conn, 'Tarification');
			foreach ($cout as $ligne) {
				if($ligne['CATEGORIE'] == 'PROF_PRIX_INDIVIDUEL'){
					$COUT_HORAIRE = $ligne['PRIX'];
				}
			}
...
<div class="col-sm-8">
	...
	<form method="POST" action="mon-espace.php?modifiercoursprofesseurs">
		<div class="row">
			<div class="col-sm-2">
			...
			<div class="col-sm-12">
				<div class="text-center">
					<a href="mon-espace.php?supprimerCoursProfesseur" class="btn  btn-bouton-gris  btn-lg">Supprimer une demande</a>
